...
// Dukung akses via GET (?id=) dan via POST (action=lampiran_manual, id_sertifikat)
$action = $_POST['action'] ?? '';
if ($action === 'lampiran_manual') {
...
$namaOverride = isset($_POST['nama_override']) ? trim((string)$_POST['nama_override']) : '';
$pelOverride = isset($_POST['pelatihan_override']) ? trim((string)$_POST['pelatihan_override']) : '';
$instrukturOverride = isset($_POST['instruktur_override']) ? trim((string)$_POST['instruktur_override']) : '';
$lampNama = $namaOverride !== '' ? $namaOverride : (string)($data['nama_lengkap'] ?? '');
$lampPelatihan = $pelOverride !== '' ? $pelOverride : (string)($data['nama_pelatihan'] ?? '');
$lampInstruktur = $instrukturOverride !== '' ? $instrukturOverride : (string)($data['nama_instruktur'] ?? '');

// Lampiran: dukung input manual (POST dari modal) dan cetak dari snapshot DB
$isLampiranManual = ($action === 'lampiran_manual');
$unitsToPrint = [];
if ($isLampiranManual) {
  $csrf = $_POST['csrf_token'] ?? '';
  if (!empty($_SESSION['csrf_token']) && hash_equals($_SESSION['csrf_token'], $csrf)) {
    $units = is_array($_POST['units'] ?? null) ? $_POST['units'] : [];
    $save_db = isset($_POST['save_db']) && (string)$_POST['save_db'] === '1';
    $norm = [];
    foreach ($units as $u) {
      $nm = trim((string)($u['nama'] ?? ''));
      $kd = trim((string)($u['kode'] ?? ''));
      if ($nm !== '' || $kd !== '') { $norm[] = ['nama' => $nm, 'kode' => $kd]; }
    }
    $unitsToPrint = $norm;
    if ($save_db) {
      try {
        $pdo->exec("
          CREATE TABLE IF NOT EXISTS tb_sertifikat_unit (
            id_sertifikat_unit INT AUTO_INCREMENT PRIMARY KEY,
            id_sertifikat INT NOT NULL,
            posisi INT NOT NULL,
            nama_unit VARCHAR(255) NOT NULL,
            kode_unit VARCHAR(64) NULL,
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            INDEX (id_sertifikat)
          ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        ");
        $del = $pdo->prepare('DELETE FROM tb_sertifikat_unit WHERE id_sertifikat = ?');
        $del->execute([$id]);
        if (!empty($unitsToPrint)) {
          $ins = $pdo->prepare('INSERT INTO tb_sertifikat_unit (id_sertifikat, posisi, nama_unit, kode_unit) VALUES (?,?,?,?)');
          foreach ($unitsToPrint as $i => $u) {
            $ins->execute([$id, $i + 1, $u['nama'] ?? '', $u['kode'] ?? null]);
          }
        }
      } catch (Throwable $e) { /* abaikan kesalahan DB untuk proses cetak */ }
    }
  }
}
// Jika tidak ada data manual, coba muat dari snapshot DB
if (empty($unitsToPrint)) {
  try {
...
<head>
  ...
  <style>
    ...
    .certificate {
      ...
      margin: 0 auto;
      padding: 14mm 16mm; /* ruang dalam aman */
      border: 8px double var(--border);
      box-shadow: 0 0 0 6px rgba(201,176,122,0.25) inset;
      box-sizing: border-box; /* sertakan border & padding dalam ukuran */
    ...
    .watermark img { max-width: 65%; filter: grayscale(100%); }
    .header {
      display: flex;
      align-items: center;
    ...
    .subtitle { text-align: center; font-size: 12pt; font-style: italic; margin-bottom: 10mm; }
    .recipient {
      text-align: center;
      font-size: 22pt;
      font-weight: 700;
    ...
    .details .item { display: flex; gap: 6px; }
    .details .label { font-weight: 700; min-width: 150px; }
    .footer {
      display: flex;
      justify-content: space-between;
    ...
    .number { font-size: 11pt; font-weight: 700; }
    .sign {
      text-align: right;
      min-width: 320px;
    }
    ...
    .sign .grade { font-size: 10pt; }
    .actions.no-print { margin: 6mm 20mm; display: flex; gap: 10px; }
    .btn { padding: 6px 12px; border: 1px solid #444; background: #f5f5f5; cursor: pointer; }
...
<body onload="window.print()">
  <div class="certificate">
    ...
    <div class="details">
      <div class="item"><div class="label">Periode</div><div class="value">: <?php echo htmlspecialchars(($data['tanggal_mulai'] ?? '') . ' s/d ' . ($data['tanggal_selesai'] ?? '')); ?></div></div>
      <div class="item"><div class="label">No Induk Pendaftaran</div><div class="value">: <?php echo htmlspecialchars($data['no_induk_pendaftaran'] ?? '-'); ?></div></div>
      <div class="item"><div class="label">Tanggal Terbit</div><div class="value">: <?php echo htmlspecialchars($data['tanggal_terbit'] ?? ''); ?></div></div>
    </div>
  ...
  <div class="certificate">
    ...
    <div class="header">
      ...
      <div class="number">
        ...
        <?php if (!empty($lampInstruktur)) { ?>
          <div style="font-size: 11pt; font-weight: normal; margin-top: 4px;">Instruktur: <?php echo htmlspecialchars($lampInstruktur); ?></div>
        <?php } ?>
      </div>
    </div>
    <div class="subtitle">Daftar Unit Kompetensi Yang Dicapai</div>
    <div class="statement">
      Pelatihan <strong><?php echo htmlspecialchars($lampPelatihan ?? ''); ?></strong>
    </div>
    <div class="section">
